...
fn basic_switch_local_traffic()
{
    ...
    let cycles = messages_per_server * message_size + 2; //+2 is because of the switch-Nic and Nic-switch links which take one cycle each
...
fn basic_two_servers_two_routers()
{
    ...
    let cycles = messages_per_server * message_size + 3; //+3 is because of the switch-Nic + switch-switch + Nic-switch links which take one cycle each
    let maximum_packet_size=16;
