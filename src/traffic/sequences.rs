...
impl Traffic for Sequence
{
    ...
    {
        self.traffics[self.current_traffic].try_consume(task,message,cycle,topology,rng)
    }
...
impl Traffic for MessageTaskSequence
{
        ...
        for i in 0..self.traffics.len() {
            if messages_sent[i] < messages_to_send_per_traffic[i] {
                ...
                let message = Rc::new(Message {
                    ...
                    payload: self.id.to_le_bytes().into(),
                });
                return Ok(message);
            }
        }
    ...
    fn try_consume(&mut self, task: usize, message: &dyn AsMessage, cycle: Time, topology: &dyn Topology, rng: &mut StdRng) -> bool {
        ...
        for i in 0..self.generated_messages.len() {
            if self.generated_messages[i].remove(&id) {
                messages_consumed[i] += 1;
                return self.traffics[i].try_consume(task, message, cycle, topology, rng);
            }
        }
