...
pub trait Traffic : Quantifiable + Debug
{
	...
	fn should_generate(&self, server:usize, _cycle:Time, rng: &mut StdRng) -> bool
	{
...
impl Traffic for Homogeneous
{
	...
	fn try_consume(&mut self, _server:usize, message: Rc<Message>, _cycle:Time, _topology:&dyn Topology, _rng: &mut StdRng) -> bool
	{
	...
	fn server_state(&self, _server:usize, _cycle:Time) -> ServerTrafficState
	{
...
impl Traffic for Burst
{
	...
	fn server_state(&self, server:usize, _cycle:Time) -> ServerTrafficState
	{
		if self.pending_messages[server]>0 {
			ServerTrafficState::Generating
		} else {
...
impl Traffic for TimeSequenced
{
	...
	fn should_generate(&self, server:usize, cycle:Time, rng: &mut StdRng) -> bool
	{
		let mut offset = cycle;
		let mut traffic_index = 0;
		while traffic_index<self.traffics.len() && offset >= self.times[traffic_index]
...
impl Sequence
{
	pub fn new(arg:TrafficBuilderArgument) -> Sequence
	{
		...
		let traffics = (0..period_number).flat_map(|_ip| traffics_args.iter().map(
...
impl Traffic for MultimodalBurst
{
	...
	fn try_consume(&mut self, server:usize, message: Rc<Message>, _cycle:Time, _topology:&dyn Topology, _rng: &mut StdRng) -> bool
	{
		let message_ptr=message.as_ref() as *const Message;
...
/// The map is applied to the traffic when created and the result is used as the traffic to be generated.
#[derive(Quantifiable)]
#[derive(Debug)]
...
impl Traffic for TrafficMap
{
	fn generate_message(&mut self, origin: usize, cycle: Time, topology: &dyn Topology, rng: &mut StdRng) -> Result<Rc<Message>, TrafficError>
	{
		// check if the origin is inside the traffic
		if origin >= topology.num_servers()
		{
			return Err(TrafficError::OriginOutsideTraffic);
		}
		...
		let destination = self.into_base_map[origin];
