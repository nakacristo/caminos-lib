...
A [SubRangeTraffic] makes tasks outise the range to not generate traffic.
```ignore
SubRangeTraffic{
	start: 100,
	end: 200,
...
/**
The tasks in a ProductTraffic are grouped in blocks of size `block_size`. The traffic each block generates follows the subyacent `block_traffic` [Traffic], but with the group of destination being indicated by the `global_pattern`.
...
The following example uses TrafficMap together with [TrafficSum], [CartesianEmbedding], [Composition], and [CartesianTransform] to divide the network into two regions, each employing a different kind of traffic.
```ignore
TrafficSum
{
	list: [
