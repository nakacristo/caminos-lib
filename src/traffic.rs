...
pub trait Traffic : Quantifiable + Debug
{
	...
	fn generate_message(&mut self, origin:usize, cycle:usize, topology:&dyn Topology, rng: &mut StdRng) -> Result<Rc<Message>,TrafficError>;
	...
	fn try_consume(&mut self, server:usize, message: Rc<Message>, cycle:usize, topology:&dyn Topology, rng: &mut StdRng) -> bool;
	...
	fn should_generate(&self, server:usize, _cycle:usize, rng: &mut StdRng) -> bool
	{
...
impl Traffic for Homogeneous
{
	...
	fn try_consume(&mut self, _server:usize, message: Rc<Message>, _cycle:usize, _topology:&dyn Topology, _rng: &mut StdRng) -> bool
	{
...
impl Traffic for TimeSequenced
{
	...
	fn should_generate(&self, server:usize, cycle:usize, rng: &mut StdRng) -> bool
	{
		let mut offset = cycle;
		let mut traffic_index = 0;
		while traffic_index<self.traffics.len() && offset >= self.times[traffic_index]
...
impl Traffic for BoundedDifference
{
	...
	fn try_consume(&mut self, server:usize, message: Rc<Message>, _cycle:usize, _topology:&dyn Topology, _rng: &mut StdRng) -> bool
	{
		let message_ptr=message.as_ref() as *const Message;
